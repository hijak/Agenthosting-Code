...
export function DialogMimoLogin() {
  ...
  const options = createMemo(() => {
    const recommended = [
      {
        title: t("tui.dialog.login.agenthosting"),
        value: "agenthosting",
        description: t("tui.dialog.login.agenthosting.desc"),
        category: "Recommended",
        onSelect: async () => {
          const result = await sdk.client.provider.oauth.authorize({
            providerID: "agenthosting",
            method: 0,
          })
          if (result.error) {
    ...
    return [
      ...recommended,
      ...providerOptions().filter((option) => option.value !== "agenthosting" && option.value !== "xiaomi"),
    ]
  })
