...
export const dict = {
  ...
  "tui.home.agreement.prefix": "AgentHosting をご利用いただくことで、",
  ...
  "tui.tips.free_api_sunset":
    "無料 API サービスは終了しました。{highlight}/login{/highlight} でログインしてください。AgentHosting Plan を購読するか、サードパーティ API を設定して AgentHosting をご利用ください。",
  ...
  "tui.tips.config_files":
    "サーバ設定は {highlight}agenthosting.json{/highlight}、TUI 設定は {highlight}tui.json{/highlight} を作成します",
  "tui.tips.global_config":
    "TUI 設定をグローバルに置くなら {highlight}~/.config/agenthosting/tui.json{/highlight} を使います",
  ...
  "tui.tips.mcp_oauth": "AgentHosting は認証が必要なリモート MCP サーバの OAuth を自動処理します",
  "tui.tips.custom_command":
    "{highlight}.agenthosting/command/{/highlight} に {highlight}.md{/highlight} ファイルを追加すると再利用可能なカスタムプロンプトを定義できます",
  ...
  "tui.tips.custom_agent":
    "{highlight}.agenthosting/agent/{/highlight} に {highlight}.md{/highlight} ファイルを追加して専用 AI ペルソナを作れます",
  ...
  "tui.tips.formatter": "AgentHosting は prettier、gofmt、ruff などでファイルを自動整形します",
  ...
  "tui.tips.lsp": "AgentHosting はインテリジェントなコード解析に LSP サーバを使います",
  "tui.tips.custom_tool":
    "{highlight}.agenthosting/tools/{/highlight} に {highlight}.ts{/highlight} ファイルを作って新しい LLM ツールを定義します",
  "tui.tips.tool_scripts": "ツール定義から Python、Go などのスクリプトを呼び出せます",
  "tui.tips.plugins":
    "{highlight}.agenthosting/plugin/{/highlight} に {highlight}.ts{/highlight} ファイルを追加してイベントフックを実装できます",
  "tui.tips.plugin_notify": "プラグインでセッション完了時に OS 通知を送れます",
  "tui.tips.plugin_protect": "プラグインを書いて AgentHosting が機密ファイルを読まないようにできます",
  "tui.tips.run": "{highlight}ah run{/highlight} で非対話的なスクリプト実行ができます",
  "tui.tips.continue": "{highlight}ah --continue{/highlight} で前回のセッションを再開します",
  "tui.tips.attach_cli": "{highlight}ah run -f file.ts{/highlight} で CLI からファイルを添付できます",
  "tui.tips.format_json": "{highlight}--format json{/highlight} を使うとスクリプトで機械可読な出力を得られます",
  "tui.tips.serve": "{highlight}ah serve{/highlight} を実行するとヘッドレスで AgentHosting API にアクセスできます",
  "tui.tips.attach_server": "{highlight}ah run --attach{/highlight} で起動中のサーバに接続します",
  "tui.tips.upgrade": "{highlight}ah upgrade{/highlight} で最新バージョンに更新します",
  "tui.tips.auth_list": "{highlight}ah providers list{/highlight} で設定済みプロバイダを確認します",
  "tui.tips.agent_create": "{highlight}ah agent create{/highlight} でガイド付きエージェント作成を行います",
  "tui.tips.github_install": "{highlight}ah github install{/highlight} で GitHub workflow を設定します",
  ...
  "tui.tips.theme_files": "{highlight}.agenthosting/themes/{/highlight} ディレクトリに JSON テーマファイルを作成します",
  ...
  "tui.tips.debug_config": "{highlight}ah debug config{/highlight} で設定の問題を調査します",
  ...
  "tui.tips.zen": "AgentHosting と組み合わせて {highlight}/connect{/highlight} で厳選されたモデルを使えます",
  ...
  "tui.skill.mimocode.description": "AgentHosting の機能・設定・コマンドに関するセルフドキュメント",
  ...
  "tui.skill.drive-mimo.description": "別の AgentHosting プロセスをプログラムで操作——ヘッドレス JSON イベントまたは tmux 経由のインタラクティブ TUI",
  ...
  "tui.dialog.free_api_sunset.message":
    "/login でログインしてください。AgentHosting Plan を購読するか、サードパーティ API を設定して AgentHosting をご利用ください。",
  ...
  "tui.model.mimo_auto.name": "（ 7月26日 18:00 まで無料 · UTC+8）",
  ...
  "tui.dialog.try_best.handoff.description": "未完了の作業をこのハーネスへ委任するよう AgentHosting に依頼",
  ...
  "tui.toast.update_available.success": "AgentHosting v{{version}} に更新しました。アプリケーションを再起動してください。",
  ...
  "tui.voice.error.no_auth": "/connect で AgentHosting アカウントに接続するか、voice.asr_model で別のプロバイダーを設定してください",
  ...
  "tui.dialog.login.mimo_free.success": " の準備完了 — デフォルトモデルを agenthosting/login に設定しました",
  "tui.dialog.login.mimo_free.unavailable": " プロバイダーが読み込まれていません",
  "tui.dialog.login.flow.title": "AgentHosting ログイン",
  ...
  "cli.providers.mimo_free.hint": "匿名無料チャネル / login",
  "cli.providers.mimo_free.verifying": " チャネルを検証中...",
  "cli.providers.mimo_free.ready": " チャネル準備完了",
  "cli.providers.mimo_free.failed": " の自己診断に失敗",
  "cli.providers.mimo_free.default_set": "デフォルトモデルを agenthosting/login に設定（1M コンテキスト、無料）",
  "cli.providers.mimo_free.usage_hint":
    "ログイン不要 — そのまま ah を実行できます。有料/上位モデルを利用する場合は AgentHosting ブラウザログインを選択してください。",
  ...
  "trust.capabilities": "AgentHosting はここでファイルの読み取り、編集、実行が可能になります。",
  ...
  "skip_permissions.body":
    "--dangerously-skip-permissions を指定して起動しました。AgentHosting は承認を求めずにファイルの読み取り・編集・実行やシェルコマンドの実行を行います。設定で明示的に拒否（deny）したルールのみが引き続き適用されます。発生したいかなる結果についてもあなた自身が全責任を負います。",
